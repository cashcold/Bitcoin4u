...
class ConfirmBuyMain extends Component {
    ...
    onSubmit = (event)=>{
        ...
        axios.post("http://localhost:8000/users/buyBitcoin/", BuyBitcoin).then(res => {toast.success("Transaction Successful")}).then(res => setTimeout(()=>{
            window.location="/dashboard"
        }),9000).catch(err => {toast.error(err.response.data)})
